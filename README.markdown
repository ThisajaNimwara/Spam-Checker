...
- **(Optional) API Keys**:
  - APILayer SpamCheck (get from [apilayer.com](https://apilayer.com)).
  - VirusTotal (get from [virustotal.com](https://virustotal.com)).
...
6. **(Optional) Install SpamAssassin**:
   ...
   - On Linux:
     ```bash
     sudo apt install spamassassin spamc && sa-update
     ```
...
2. Create a `Procfile` (example):
   ```
   web: gunicorn -w 2 src.app:app
   ```

3. Ensure the `Model/` directory is included and API keys are set as environment variables.
4. **Note**: VirusTotal's free API has rate limits; cache results in production.
